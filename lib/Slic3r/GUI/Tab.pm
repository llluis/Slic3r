...
sub build {
    ...
    $self->init_config_options(qw(
        ...
        retract_length retract_lift retract_speed unretract_speed retract_restart_extra retract_before_travel retract_layer_change wipe
        retract_length_toolchange retract_restart_extra_toolchange pressure_multiplier
    ));
...
sub _extruder_options { qw(nozzle_diameter extruder_offset retract_length retract_lift retract_speed retract_restart_extra retract_before_travel wipe
    unretract_speed retract_layer_change retract_length_toolchange retract_restart_extra_toolchange pressure_multiplier) }
    ...
    foreach my $extruder_idx (@{$self->{extruder_pages}} .. $self->{extruders_count}-1) {
        ...
        }
        {
            ...
            $optgroup->append_single_option_line($_, $extruder_idx)
                for qw(retract_length retract_lift retract_speed unretract_speed retract_restart_extra retract_before_travel retract_layer_change wipe);
        }
        {
        ...
        {
            my $optgroup = $page->new_optgroup('Pressure Control (advanced settings for bowden setups)');
            $optgroup->append_single_option_line($_, $extruder_idx)
                for qw(pressure_multiplier);
        }
